...
func TestPairs(t *testing.T) {
	...
	if !slice.ContentEquivalent(pairs.Firsts(), []string{"1", "2", "3", "4"}) || !slice.ContentEquivalent(pairs.Seconds(), []int{1, 2, 3, 4}) {
		t.Error("Error: Values are not equal !")
	}

	_0, _1 := pairs.Split()
	if !slice.ContentEquivalent(_0, pairs.Firsts()) || !slice.ContentEquivalent(_1, pairs.Seconds()) {
		t.Error("Error: Values are not equal !")
	}
