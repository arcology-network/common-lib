...
func (this *Entry[T]) Size() uint64 {
	...
	if this.SizeOf != nil {
		this.sizeInMem = this.SizeOf(this.Value)
	}
...
func NewStore[K comparable, T any](backend KVStore[K, *Entry[T]], cacheCap uint64) *CachedKVStore[K, T] {
	return &CachedKVStore[K, T]{
		...
		backend: backend,
		cachePolicy: NewCachePolicy(cacheCap, func(v *Entry[T]) uint64 {
			if v == nil {
				return 0
			}
			return v.Size()
