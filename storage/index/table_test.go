...
func TestTableAlone(t *testing.T) {
	...
	txs := common.Append(make([]*Tx, 10), func(i int, tx *Tx) *Tx {
...
func TestTableWithDB(t *testing.T) {
	...
	keys := common.Append(txs, func(_ int, tx *Tx) string {
		return string(tx.Hash[:])
	})

	// Encode the transactions.
	encoded := common.Append(txs, func(_ int, tx *Tx) []byte {
	...
	queryTxs := common.Append(encoded, func(i int, data []byte) *Tx {
		tx := &Tx{}
	...
	indics := common.Append(txs, func(i int, tx *Tx) *txIndex {
		return &txIndex{
			time:       tx.Time,
			height:     tx.Height,
			primaryKey: tx.Hash,
	...
	primaryKeys := common.Append(res, func(i int, tx *txIndex) string {
