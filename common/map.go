...
func MapFromSlice[K comparable, V any](keys []K, v V) *map[K]V {
	M := make(map[K]V)
	for _, k := range keys {
		M[k] = v
	}
...
// MapFromSliceBy creates a map from an array of values, with keys generated by a getter function,
// and all values set to a specified initial value.
func MapFromSliceBy[K comparable, T, V any](keys []T, initv V, getter func(t T) K) *map[K]V {
	M := make(map[K]V)
	for _, k := range keys {
		M[getter(k)] = initv
	}
