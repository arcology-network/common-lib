...
// CastTo casts a value to a different type using a predicate function.
func CastTo[T0, T1 any](src T0, predicate func(T0) T1) T1 {
	return predicate(src)
}

// ArrayCastTo casts each element in a slice to a different type using a predicate function.
...
// ToInterfaces converts each element in a slice to an interface{} type.
// It returns a new slice with the converted elements.
...
// It takes an array and a getter function that returns a pointer to the key for each element.
// It returns two slices, one containing the group indices for each element and the other containing the number of groups.
...
// It takes a map, source, and a condition function that determines whether a key-value pair should be removed.
...
// It takes two maps, source and target, and a condition function that determines whether a key-value pair should be moved.
...
// It takes two maps, from and to, and returns the merged map.
...
// It takes an array of keys and a value, and returns a pointer to the created map.
...
// It takes an array of values, an initial value, and a getter function that returns the key for each value.
// It returns a pointer to the created map.
...
// It takes a map, m, and returns a slice containing all the keys of the map.
...
// It takes a map, m, and returns a slice containing all the values of the map.
...
// The input map must have keys of type K and values of type V.
// The returned slices will have the same length as the input map.
...
// It takes a pair of elements, v0 and v1, and returns v0.
func FilterFirst[T0, T1 any](v0 T0, v1 T1) T0 { return v0 }

// FilterSecond returns the second element of a pair.
// It takes a pair of elements, v0 and v1, and returns v1.
